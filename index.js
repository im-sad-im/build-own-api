...
app.get("/jokes/:id", (req, res) => {
  //  req.param for id endpoints (Mapped to the named route "parameters")
...
app.get("/filter", (req, res) => {
  ...
  const filteredJoke = jokes.filter((joke) => joke.jokeType === query);
...
app.put("/jokes/:id", (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id == newId);
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(404).json({
      error: `Item with id : ${id} is not found, No joke were deleted`,
    });
  }
});
